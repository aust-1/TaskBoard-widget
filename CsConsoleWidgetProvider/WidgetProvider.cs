...
public sealed class WidgetProvider : IWidgetProvider
{
                ...
                foreach (var widgetInfo in widgetManager.GetWidgetInfos())
                {
                    ...
                    if (!WidgetInstances.ContainsKey(context.Id))
                    {
                        if (WidgetImpls.ContainsKey(context.DefinitionId))
                        {
                            ...
                            WidgetInstances[context.Id] = WidgetImpls[context.DefinitionId]
                                (context.Id, widgetInfo.CustomState);
                        }
                        else
                        {
    ...
    {
        [CountingWidget.DefinitionId] = (widgetId, initialState) =>
            new CountingWidget(widgetId, initialState),
        [WeatherWidget.DefinitionId] = (widgetId, initialState) =>
            new WeatherWidget(widgetId, initialState),
    };
    ...
    {
        Console.WriteLine(
            $"CreateWidget id: {widgetContext.Id} definitionId: {widgetContext.DefinitionId}"
        );

        if (!WidgetImpls.ContainsKey(widgetContext.DefinitionId))
        {
            Console.WriteLine(
                $"ERROR: Requested unknown Widget Definition ${widgetContext.DefinitionId}"
            );
    ...
    {
        Console.WriteLine(
            $"OnActionInvoked id: {actionInvokedArgs.WidgetContext.Id} definitionId: {actionInvokedArgs.WidgetContext.DefinitionId}"
        );
    ...
    {
        Console.WriteLine(
            $"OnWidgetContextChanged id: {contextChangedArgs.WidgetContext.Id} definitionId: {contextChangedArgs.WidgetContext.DefinitionId}"
        );
        WidgetInstances[contextChangedArgs.WidgetContext.Id]
    ...
    {
        Console.WriteLine(
            $"Activate id: {widgetContext.Id} definitionId: {widgetContext.DefinitionId}"
        );
